"""
source_evidence_to_training_asset_builder.py を完全に修正
source_evidence の transcript_segments をクリーンアップ
"""
...
class SourceEvidenceToTrainingAssetBuilder:
    ...
    def load_source_evidence(self, file_path: Path) -> dict:
        ...
        # Clean up transcript_segments: keep only required fields
        if "transcript_segments" in data:
            ...
            for ts in data.get("transcript_segments", []):
                ...
                if "confidence" in ts:
                    cleaned["confidence"] = ts["confidence"]
                cleaned_segments.append(cleaned)
            data["transcript_segments"] = cleaned_segments
...
print("✓ source_evidence_to_training_asset_builder.py を完全修正しました")
